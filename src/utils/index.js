...
export const getLaureateFullName = (laureate) => {
  ...
  testdict[fullname] = (testdict[fullname] || 0) + 1;
...
export const getdict = () => {
  setTimeout(() => {
    Object.entries(testdict).forEach(([key, value]) => {
      if (
        value > 2 &&
        !(key.includes('Office') || key.includes('International'))
      )
        nobels.push(key);
    });
...
export const getLaureatesNames = (laureates, listOfFullNames) => {
  let laureatesNames = '';
  const laureatesTotal = laureates?.length || 0;
  if (laureatesTotal > 0) {
    laureates.forEach((laureate, idx) => {
      const fullName = listOfFullNames
        ? laureate
        : getLaureateFullName(laureate);
